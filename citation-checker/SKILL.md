...
description: Verifies every entry in a .bib file against CrossRef, Semantic Scholar, and OpenAlex to detect hallucinated, chimeric (real title + wrong authors), or mis-attributed references. Use after writing or editing any .bib file, before paper submission, when the user asks to check/verify/audit citations or references, or whenever new BibTeX entries are added by the lit-review or scientific-writing workflows. Requires python3 + requests; no API keys.
---
